...
import ss.pku.attacktraceproject.networkFlowWatermarking.wmjp.db.WmCheckStorePacketThreadInfo;

import java.util.ArrayList;
import java.util.List;

public class TcpPacketThreadHandler {
    static List<Long> receivedTimeSet = new ArrayList<Long>();

    public TcpPacketThreadHandler() {
...
/*        System.out.println("---------------TcpPacketHandler.handler().start----------------");
   ...
   /*     if (tcpPacket.version == 4 && tcpPacket.src_ip.toString().equals(ProjectConstants.ORIGINAL_SENDER_SOURCE_IP)) {
        ...
        //1、记录收包时间
        long receivedPacketTime=System.currentTimeMillis();
        String receivedPacketTimeStr=String.valueOf(receivedPacketTime);
        //2、对收到的包进行存库处理
        System.out.println("水印提取检测方开始对收到的包进行存库处理。");
        WmCheckStorePacketThreadInfo.storePacketInfo(tcpPacket,receivedPacketTimeStr);
